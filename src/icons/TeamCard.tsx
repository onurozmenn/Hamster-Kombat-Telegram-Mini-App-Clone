...
type TeamCardProps = {
    ...
    totalProfit: string;
    isLocked?: boolean;
    onClickEvent:  React.MouseEvent<HTMLDivElement>;
};
    ...
    <div className="bg-[#292c34] rounded-lg p-4 w-full relative flex flex-col items-center">
        ...
        {!isLocked && (
            <>
                ...
                <p className="text-yellow-400 font-bold">{profitPerHour} ₺</p>
                <p className="text-gray-400 text-sm mt-2">lvl {level}</p>
                <button onClick={onClickEvent} className="text-white rounded p-1 px-4 mt-1 bg-[#43433b99] font-bold">{totalProfit} ₺</button>
            </>
        )}
    </div>
);